<template>
  <div
    ...
    :class="{
      ...
      'hover:border-primary-600': layerStore.currentLayer?.name !== layer.name,
    ...
    class="relative border-2 cursor-pointer"
    tabindex="0"
    @click="setFocus($event)"
  >
    <div
      v-if="layerStore.showingLayerNames"
      class="text-xs p-1 px-2 rounded-md text-slate-400 inline-flex"
    >
      {{ `Layer ` + layer.name }}
    </div>
...
const cssClasses = (
  defaults: string,
  selectedClasses: { [name: string]: string }
) => {
