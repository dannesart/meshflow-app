<template>
  <div class="relative">
    <!-- Color -->
    <div
      v-if="type === 'text-color' || type === 'bg-color'"
    ...
    >
      <div
        class="w-10 h-10 border-2 border-white rounded-full cursor-pointer flex items-center justify-center hover:shadow-lg"
        :class="`bg-${value}`"
      >
        <p class="text-sm">
      ...
      <div
        class="absolute top-full mt-2 flex flex-col gap-3 p-3 bg-white shadow-xl rounded-lg"
        v-if="inputFocused"
      >
        ...
        <div class="flex gap-3">
          <div
            ...
            class="w-7 h-7 shadow-lg rounded-full cursor-pointer"
            :class="{
              'border-2 border-primary-500':
                (type === 'text-color' ? 'text-' : 'bg-') + color === value,
              [`bg-${color}`]: true,
            }"
    ...
    >
      ...
      >
        ...
        <div class="flex gap-3 w-40 flex-col flex-wrap">
          ...
          >
            <div
              class="w-7 h-7 shadow-lg rounded-full flex items-center justify-center"
            >
              <div
                name="radio-value-selected"
                class="w-4 h-4 bg-primary-600 rounded-full"
                v-if="radioValue === value"
              ></div>
            </div>
            {{ radioValue }}
          </div>
...
const { type, value, values, disabled, max, min, label, icon } = defineProps([
  "type",
  "value",
  "values",
  "disabled",
...
const setRangeFocus = () => {
  setFocus();
  setTimeout(() => {
    if (rangeInput && rangeInput.value) rangeInput.value.focus();
  }, 100);
};
const setBlured = () => {
